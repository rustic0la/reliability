  ...
  mx.mxEvent.addListener(button, 'click', () => {
    switch (action) {
      ...
      case 'vertical':
        ...
        if (alignVerticallyCells.length > 0) {
          ...
          alignVerticallyCells?.map(
            (cell) => (cell.geometry.x = centerX - cell.geometry.width / 2),
          );
          graph.refresh();
        }
      ...
      case 'horizontal':
        ...
        if (alignHorizontallyCells.length > 0) {
          ...
          alignHorizontallyCells.map(
            (cell) => (cell.geometry.y = centerY - cell.geometry.height / 2),
          );
          graph.refresh();
        }
        break;
